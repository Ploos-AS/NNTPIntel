...
def risk_page(storage: Storage) -> str:
    ...
    ) or '<tr><td colspan="8">No topology risk data yet.</td></tr>'
    community_rows = "".join(
        "<tr>"
    ...
    ) or '<tr><td colspan="7">No topology communities yet.</td></tr>'
...
<p><a href="/web/propagation/topology">← Inferred topology</a> · <a href="/web/propagation/topology/resilience">Resilience</a> · <a href="/web/propagation/topology/incidents">Incidents</a></p>
<h2>Topology risk synthesis</h2>
<p class="warning"><strong>Triage only.</strong> {escape(data["disclaimer"])}</p>
<div class="cards"><div class="card">Incident scope<div class="metric">{escape(str(data["incident_scope"]))}</div></div><div class="card">High-risk servers<div class="metric">{data["high_risk_server_count"]}</div></div><div class="card">High-risk communities<div class="metric">{data["high_risk_community_count"]}</div></div></div>
<h3>Server priority</h3><table><thead><tr><th>Server</th><th>Cluster</th><th>Risk</th><th>Level</th><th>Incidents</th><th>Impact</th><th>Resilience</th><th>Gateway</th></tr></thead><tbody>{server_rows}</tbody></table>
<h3>Community priority</h3><table><thead><tr><th>Cluster</th><th>Risk</th><th>Level</th><th>Servers</th><th>Incidents</th><th>Affected servers</th><th>Gateways</th></tr></thead><tbody>{community_rows}</tbody></table>
</main></body></html>"""
