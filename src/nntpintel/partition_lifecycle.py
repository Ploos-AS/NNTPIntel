...
def _postgres_timestamptz_literal(value: datetime.datetime) -> str:
    """Return a deterministic PostgreSQL TIMESTAMPTZ literal for DDL.

    Partition bounds are grammar elements in CREATE TABLE and cannot use bind
    parameters. Values passed here are normalized datetimes generated by this
    module, so the rendered literal contains only a fixed numeric UTC format.
    """

    value = _utc(value)
    return f"TIMESTAMPTZ '{value:%Y-%m-%d %H:%M:%S}+00'"
...
def ensure_monthly_partition(conn, *, table: str, month: datetime.datetime) -> str:
    ...
    start_literal = _postgres_timestamptz_literal(start)
    end_literal = _postgres_timestamptz_literal(end)
    conn.execute(
