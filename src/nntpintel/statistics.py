...
def _statistics_filter(window: StatisticsRange, server_id: int | None) -> tuple[list[str], list[object]]:
    ...
    if server_id is not None:
        where.append("r.server_id = %s"); params.append(server_id)
    return where, params


def _statistics_range_payload(window: StatisticsRange) -> dict:
    return {"start": _json_value(window.start), "end": _json_value(window.end), "all_time": window.start is None}
...
def propagation_statistics(storage: StorageBackend, *, resolution: str, start: datetime.datetime | None = None, end: datetime.datetime | None = None, server_id: int | None = None) -> dict:
    ...
    with storage.connect() as conn: rows = _json_rows(conn.execute(summary_query, tuple(params)).fetchall()); values = _json_rows(conn.execute(value_query, tuple(params)).fetchall()); campaigns = _json_rows(conn.execute(campaign_query, tuple(campaign_params)).fetchall())
