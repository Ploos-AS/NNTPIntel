...
def communities_page(storage: Storage) -> str:
    ...
    ) or '<tr><td colspan="7">No inferred topology communities yet.</td></tr>'
...
<p><a href="/web/propagation/topology">← Inferred topology</a> · <a href="/web/propagation/topology/impact">Impact / centrality</a></p>
<h2>Topology communities / clusters</h2>
<p class="warning"><strong>Inference only.</strong> {escape(data["disclaimer"])}</p>
<div class="cards"><div class="card">Communities<div class="metric">{data["community_count"]}</div></div><div class="card">Isolated servers<div class="metric">{data["isolated_server_count"]}</div></div></div>
<table><thead><tr><th>Cluster</th><th>Servers</th><th>Edges</th><th>Members</th><th>Avg confidence</th><th>Avg impact</th><th>Max impact</th></tr></thead><tbody>{rows}</tbody></table>
</main></body></html>"""
