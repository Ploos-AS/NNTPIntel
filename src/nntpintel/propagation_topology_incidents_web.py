...
def topology_incidents_page(storage: Storage) -> str:
    ...
    rows = "".join(
        ...
        "</tr>"
        for item in incidents
    ) or '<tr><td colspan="8">No topology incidents recorded.</td></tr>'
...
<p><a href="/web/propagation/topology">← Inferred topology</a> · <a href="/web/propagation/topology/anomalies">Anomalies</a></p>
<h2>Topology incidents</h2>
<p class="warning"><strong>Inference only.</strong> These incidents describe persistent lifecycle state derived from NNTPIntel's observed first-seen topology inference. They do not prove direct NNTP peering or feed relationships.</p>
...
<table><thead><tr><th>ID</th><th>Server</th><th>Type</th><th>Severity</th><th>Status</th><th>Started</th><th>Updated</th><th>Resolved</th></tr></thead><tbody>{rows}</tbody></table>
</main></body></html>"""
