...
class APIHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path == "/web/propagation/topology/incidents":
            from nntpintel.propagation_topology_incidents_web import topology_incidents_page
            self._send_html(topology_incidents_page(self.storage)); return
        ...
        if path == "/web/propagation/topology/resilience":
            from nntpintel.propagation_topology_resilience_web import resilience_page
            self._send_html(resilience_page(self.storage)); return
        if path == "/web/propagation/topology/risk":
            from nntpintel.propagation_topology_risk_web import risk_page
            self._send_html(risk_page(self.storage)); return
        ...
        if path == "/api/v1/statistics/servers":
            from nntpintel.statistics_api import server_statistics_request
            try:
                payload = server_statistics_request(self.storage, params)
        ...
        if path == "/api/v1/statistics/groups":
            from nntpintel.statistics_api import group_statistics_request
            try:
                payload = group_statistics_request(self.storage, params)
            except ValueError as exc:
                self._send_json({"error": str(exc)}, HTTPStatus.BAD_REQUEST); return
            except RuntimeError as exc:
                self._send_json({"error": str(exc)}, HTTPStatus.SERVICE_UNAVAILABLE); return
            self._send_json(payload); return
        if path == "/api/v1/statistics/protocol":
            from nntpintel.statistics_api import protocol_statistics_request
            try:
                payload = protocol_statistics_request(self.storage, params)
            except ValueError as exc:
                self._send_json({"error": str(exc)}, HTTPStatus.BAD_REQUEST); return
            except RuntimeError as exc:
                self._send_json({"error": str(exc)}, HTTPStatus.SERVICE_UNAVAILABLE); return
        ...
        if path == "/api/v1/statistics/propagation":
            ...
            try:
                ...
                self._send_json({"error": str(exc)}, HTTPStatus.SERVICE_UNAVAILABLE); return
            self._send_json(payload); return
        if path == "/api/v1/statistics/topology":
            from nntpintel.statistics_api import topology_statistics_request
            try:
                payload = topology_statistics_request(self.storage, params)
            except ValueError as exc:
                self._send_json({"error": str(exc)}, HTTPStatus.BAD_REQUEST); return
            except RuntimeError as exc:
                self._send_json({"error": str(exc)}, HTTPStatus.SERVICE_UNAVAILABLE); return
