...
def topology_page(storage: Storage) -> str:
    ...
    ) or '<tr><td colspan="3" class="muted">No topology nodes yet.</td></tr>'
    edge_rows = "".join(
        "<tr>"
    ...
    ) or '<tr><td colspan="8" class="muted">No inferred edges meet the confidence threshold.</td></tr>'
...
<p><a href="/web/propagation">← Propagation</a> · <a href="/web/propagation/topology/overview">Executive overview</a> · <a href="/web/propagation/topology/quality">Data quality / confidence</a> · <a href="/web/propagation/topology/evidence">Evidence provenance</a> · <a href="/web/propagation/topology/history">Topology history / stability</a> · <a href="/web/propagation/topology/anomalies">Topology anomalies</a> · <a href="/web/propagation/topology/incidents">Topology incidents</a> · <a href="/web/propagation/topology/impact">Impact / centrality</a> · <a href="/web/propagation/topology/communities">Communities / clusters</a> · <a href="/web/propagation/topology/cross-cluster">Cross-cluster</a> · <a href="/web/propagation/topology/resilience">Resilience / critical paths</a> · <a href="/web/propagation/topology/risk">Risk synthesis</a></p><h2>Inferred propagation topology</h2>
...
<section><h2>Observed precedence edges</h2><table><thead><tr><th>Earlier server</th><th></th><th>Later server</th><th>Directional samples</th><th>Forward / reverse / ties</th><th>Confidence</th><th>Median lead s</th><th>Evidence ref</th></tr></thead><tbody>{edge_rows}</tbody></table></section>
<section><h2>Servers</h2><table><thead><tr><th>ID</th><th>Host</th><th>Evidence ref</th></tr></thead><tbody>{node_rows}</tbody></table></section>
</main></body></html>"""
